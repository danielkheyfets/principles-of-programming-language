...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv =>
    isEmpty(A.parsedToClassExps(parsed))? E.makeEmptyTEnv():
    extendEnvPair(A.parsedToClassExps(parsed),E.makeEmptyTEnv())

const extendEnvPair = (clArr: A.ClassExp[], tenv: E.TEnv): E.TEnv=>{
    if(isEmpty(clArr))
         return tenv
    else{
        const en = E.makeExtendTEnv([first(clArr).typeName.var], [A.classExpToClassTExp(first(clArr))],tenv)
        return extendEnvPair(rest(clArr),en)
    }
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const Res : Result<true> = bind(typeofExp(exp.val,tenv), (valTE: T.TExp) => checkEqualType( exp.var.texp,valTE,exp))
    return bind(Res,_=> makeOk(T.makeVoidTExp()))
};
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => 
    ...
    A.isDefineExp(exp) ? bind(makeOk(E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv)),
                         (extendedEnv:E.ExtendTEnv)=> bind(typeofDefine(exp,extendedEnv), _=> typeofProgramExps(first(exps),rest(exps),extendedEnv))): 
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const res : Result<true> = bind(typeofExp(exp.val,tenv), (valTE: T.TExp) => bind(typeofExp(exp.var,tenv), (varTE : T.TExp) => checkEqualType( varTE,valTE,exp)))
    return bind(res,_=> makeOk(T.makeVoidTExp()))
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const typename_check: Result<true> = checkEqualType(exp.typeName, A.classExpToClassTExp(exp), exp) //check that the typename is correct
    const fieldTExpressions: T.TExp[] = R.map((field: A.VarDecl) => field.texp, exp.fields); //all the type expressions of the class's fields
    const newEnv: E.TEnv = E.makeExtendTEnv(R.map((field: A.VarDecl) => field.var, exp.fields), fieldTExpressions, tenv) //extend the environment
    const methods_type_result: Result<true[]> = bind(typename_check,_=> // if result is good then
             zipWithResult((method_typeExp: T.TExp, method: A.CExp) =>  // check equal types with the methods and its names in the new environment
                            bind(typeofExp(method,newEnv), (typeExp: T.TExp) => checkEqualType(typeExp, method_typeExp, exp)),
        R.map(m => m.var.texp, exp.methods), R.map(m => m.val, exp.methods))); //params for zipWith : methods type(mabe undefiend at first), the CEXP methods
    return bind(methods_type_result, () => makeOk(T.makeProcTExp(fieldTExpressions, exp.typeName))); //[t1 * ... * tn -> type] return a prov expression 
};
