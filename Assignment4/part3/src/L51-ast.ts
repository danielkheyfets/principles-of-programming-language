...
import { concat, chain, join, map, zipWith, is, none, filter } from "ramda";
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>
    !isIdentifier(typeName) ? makeFailure('name is not identifier'):
    !isGoodBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
    safe2((vardecs: VarDecl[],bngs: Binding[])=> makeOk(makeClassExp(makeTVar(typeName),vardecs,bngs)))
                    (isArray(varDecls)? mapResult(parseVarDecl2, varDecls): makeFailure('vardec is not array'),parseBindings(bindings))  
    



const parseVarDecl2 = (sexp: Sexp): Result<VarDecl> => {
    if(!isConcreteVarDecl(sexp))
        return makeFailure('not concrete')
    if (isString(sexp)) {
        return makeOk(makeVarDecl(sexp, makeFreshTVar()));
    } else if (isArray(sexp)) {
        const v = sexp[0];
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => 
    isProgram(p)? p.exps.filter(isClassExp):
    isClassExp(p) ? [makeClassExp(p.typeName,p.fields,p.methods)]:
    [];
