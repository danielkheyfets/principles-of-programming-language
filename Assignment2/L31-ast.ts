...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> =>
    isGoodBindings(methods) && isArray(fields) && allT(isString, fields) ?
     bind(mapResult(parseL3CExp, map(second,methods)),(allLambdas: CExp[])=> makeOk(makeClassExp(map(makeVarDecl,fields),zipWith(makeBinding,map(m=>m[0],methods),allLambdas)))):
    makeFailure(`Invalid vars or methods for ClassExp`);    
