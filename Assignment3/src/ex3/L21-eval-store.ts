...
import { indexOf, is, lastIndexOf, map, reduce, repeat, zipWith } from "ramda";
...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore, ExtEnv,  theGlobalEnv, globalEnvAddBinding, theStore, applyEnvStore} from "./L21-env-store";
...
import { Result, bind, safe2, mapResult, makeFailure, makeOk, either, isOk, isFailure } from "../shared/result";
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses :number[] = get_new_Addresses(args);
...
const evalDefineExps = (def: DefineExp, exps: Exp[]): Result<Value> =>
    ...
    (rhs: Value) => {
        const s = extendStore(theStore,rhs);
        globalEnvAddBinding(def.var.var,(unbox(s.vals)).length-1);
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses :number[] = get_new_Addresses(vals);
...
const get_new_Addresses = (vals: Value[]) : number[]=>
    map((value: Value)=> {
        const s = extendStore(theStore,value);
        return unbox(s.vals).length-1;
    },vals);